...
import { saveMCQ, saveQuizResult, getQuizHistory } from '../services/database';
        ...
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            ...
            if (user) {
                if (storedScore && storedTotalQuestions && storedDownloadableMcqs) {
                    ...
                    // Call saveMCQ and saveQuizResult here to store data in Firebase
                    mcqs.forEach((mcq: Mcq) => {
                        if (mcq.options) {
                            saveMCQ(mcq.question, Object.values(mcq.options), mcq.answer);
                        }
                    });
                    saveQuizResult(parsedScore, parsedTotalQuestions, mcqs);

                    // Clear local storage after saving to Firebase
                ...
                try {
                    ...
                    setPastQuizzes(history as QuizResult[]);
                } catch (error) {
        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-4 space-y-8">
            ...
            {pastQuizzes.length > 0 && (
                <div className="w-full max-w-lg p-8 rounded-2xl glow-border backdrop-blur-md bg-white/5 shadow-xl text-white">
                    <h2 className="text-3xl font-bold tracking-wider mb-6 text-center">Your Quiz History</h2>
                    <ul className="space-y-4">
                        {pastQuizzes.map((quiz, index) => (
                            <li key={index} className="p-4 rounded-xl bg-white/5 border border-gray-700 hover:bg-white/10 transition-colors duration-200 flex justify-between items-center">
                                <div>
                                    <p className="text-xl font-semibold">Score: {quiz.score} / {quiz.totalQuestions}</p>
                                    <p className="text-sm text-gray-400">Date: {new Date(quiz.timestamp.seconds * 1000).toLocaleDateString()}</p>
                                    <p className="text-sm text-gray-400">Percentage: {((quiz.score / quiz.totalQuestions) * 100).toFixed(0)}%</p>
                                </div>
                                <button
